...
def sortkey(strength, maxlength):
    ...
    def read(self, n=-1):
        ...
        for i, r in enumerate(self._ranges):
            if self._offset in r:
                file_index = i
                actual_offset = self._offset - r[0]
                break
        result = b''
        if (n == -1 or n is None):
            to_read = self.size
...
class KeydItemDict(object):
    ...
    def __init__(self, lst, strength, maxlength=None):
        self.lst = lst
        self.sortkey = sortkey(strength, maxlength)
...
class StructReader:
    ...
    def _read_text(self, len_spec):
        ...
        if len(byte_string) == max_len:
            terminator = byte_string.find('\x00')
            if terminator > -1:
...
class Slob(Sequence):
    ...
    def __getitem__(self, i):
        ...
        read_func = lru_cache(maxsize=lru_cache_size)(read_func)
...
class BinMemWriter:
    ...
    def finalize(self, fout, compress):
        count = len(self)
...
class RefList(ItemList):
    ...
    def __init__(self, f, encoding, offset=0, count=None):
        ItemList.__init__(self, StructReader(f, encoding),
                         offset,
...
class Bin(ItemList):
    ...
    def __init__(self, count, bin_bytes):
        ItemList.__init__(self, StructReader(io.BytesIO(bin_bytes)),
                         0,
                         count,
                         U_INT)
...
class Store(ItemList):
    ...
    def __init__(self, file_, offset, decompress, content_types):
        ItemList.__init__(self, StructReader(file_),
                         offset,
                         U_INT,
                         U_LONG_LONG,
                         cache_size=32)
...
class Writer(object):
    ...
    def __init__(self,
        ...
        self.tmpdir = tmpdir = TemporaryDirectory(
    ...
    def _sort(self):
        ...
        with MultiFileReader(self.f_ref_positions.name, self.f_refs.name) as f:
            ...
            sortkey_func = sortkey(IDENTICAL, None)
...
class TestReadWrite(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tmpdir = TemporaryDirectory(prefix='test')
...
def _p(i, *args, **kwargs):
    step = kwargs.pop('step', 100)
    steps_per_line = kwargs.pop('steps_per_line', 50)
    fmt = kwargs.pop('fmt', '{}')
